...
void CAN_Hardware_Init(void)
{
    ...
    uint32_t ID1 = 0x123u << 21;
    uint32_t ID2 = 0x12345678u << 3 | 0x4u;
    /* CAN 过滤器初始化（配置为列表模式）*/
    CAN_FilterInit(&(CAN_FilterInitTypeDef) {
        .CAN_FilterNumber = 0,                      // 过滤器组0
        .CAN_FilterIdHigh = ID1 >> 16,                 // 过滤器 ID 高 16 位
        .CAN_FilterIdLow = ID1,                  // 过滤器 ID 低 16 位
        .CAN_FilterMaskIdHigh = ID2 >> 16,             // 过滤器掩码高 16 位
        .CAN_FilterMaskIdLow = ID2,              // 过滤器掩码低 16 位
