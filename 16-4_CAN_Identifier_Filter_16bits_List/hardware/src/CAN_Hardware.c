...
void CAN_Hardware_Init(void)
{
    ...
    CAN_Init(CAN1, &(CAN_InitTypeDef) {
        .CAN_Mode = CAN_Mode_LoopBack,    // 环回模式
    ...
    CAN_FilterInit(&(CAN_FilterInitTypeDef) {
        ...
        .CAN_FilterIdHigh = 0x234 << 5,                 // 过滤器 ID 高 16 位
        .CAN_FilterIdLow = 0x345 << 5,                  // 过滤器 ID 低 16 位
        .CAN_FilterMaskIdHigh = 0x567 << 5,             // 过滤器掩码高 16 位
        .CAN_FilterMaskIdLow = 0x0000 << 5,              // 过滤器掩码低 16 位
        .CAN_FilterScale = CAN_FilterScale_16bit,   // 16 位过滤器
        .CAN_FilterMode = CAN_FilterMode_IdList,    // 列表模式
