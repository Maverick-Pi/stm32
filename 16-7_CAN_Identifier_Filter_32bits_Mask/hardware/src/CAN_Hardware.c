...
void CAN_Hardware_Init(void)
{
    ...
    uint32_t id = 0x12345600u << 3 | 0x4u;
    uint32_t mask = 0x1FFFFF00u << 3 | 0x4u | 0x2u;
    /* CAN 过滤器初始化（配置为列表模式）*/
    CAN_FilterInit(&(CAN_FilterInitTypeDef) {
        .CAN_FilterNumber = 0,                      // 过滤器组0
        .CAN_FilterIdHigh = id >> 16,                 // 过滤器 ID 高 16 位
        .CAN_FilterIdLow = id,                  // 过滤器 ID 低 16 位
        .CAN_FilterMaskIdHigh = mask >> 16,             // 过滤器掩码高 16 位
        .CAN_FilterMaskIdLow = mask,              // 过滤器掩码低 16 位
